...
Shader "Shader Forge/Lit_PBR" {
    Properties {
        ...
        _Glow_Power ("Glow_Power", Range(0, 200)) = 0
...
////// Specular:
                ...
                float3 diffuseColor = lerp( node_6343, (node_6343*_specular_texture_var.rgb), _use_specular_texture ); // Need this for specular when using metallic
...
////// Emissive:
                float3 emissive = lerp( 0.0, (pow(1.0-max(0,dot(normalDirection, viewDirection)),_Glow_Power)*_Glow_Color.rgb*_Glow_Color.a), _use_Glow );
/// Final Color:
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                o.Emission = lerp( 0.0, (pow(1.0-max(0,dot(normalDirection, viewDirection)),_Glow_Power)*_Glow_Color.rgb*_Glow_Color.a), _use_Glow );
                
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float3 node_6343 = (_MainTex_var.rgb*_Color.rgb);
                float4 _specular_texture_var = tex2D(_specular_texture,TRANSFORM_TEX(i.uv0, _specular_texture));
                float3 diffColor = lerp( node_6343, (node_6343*_specular_texture_var.rgb), _use_specular_texture );
