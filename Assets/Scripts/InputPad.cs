...
public class InputPad : MonoBehaviour, IDragHandler, IPointerUpHandler, IPointerDownHandler
{
    ...
    public enum RotInput { AccelAndSteer, Accelerate, Steer, View }
    public RotInput enumRotInput = RotInput.Steer;

    [Header("TapFn")]
    public int countTap;
    public int currentCountTap = 0;
    public float timeOutTap;
    public float timeOutIsTap;
    ...
    public virtual void OnPointerDown(PointerEventData ped)
    {
        ...
        if (useTap)
        {
            ...
            if (countTap <= currentCountTap)
            {
                isTap = true;
                StopAllCoroutines();
                StartCoroutine(StartCountOutIsTap(timeOutIsTap));
                SetZeroValue();
